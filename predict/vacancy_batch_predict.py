...
# -*- coding: utf-8 -*-
"""
VacancyPredict ML - Streamlit App COMPATIBLE
Automáticamente detecta y adapta features al modelo cargado
"""
...
def calc_all_possible_features(positions, box_size):
    """
    Calcula TODOS los features posibles.
    Luego seleccionaremos solo los que el modelo necesita.
    """
    features = {}
    ...
    if len(positions) >= 4:
        try:
            ...
            features['hull_volume'] = float(hull.volume)
            features['hull_area'] = float(hull.area)
        except:
            features['hull_volume'] = np.nan
            features['hull_area'] = np.nan
    else:
        features['hull_volume'] = np.nan
        features['hull_area'] = np.nan
    ...
    if len(positions) >= 3:
        ...
        features['moi_principal_1'] = float(eigenvalues[0])
        features['moi_principal_2'] = float(eigenvalues[1])
        features['moi_principal_3'] = float(eigenvalues[2])
    else:
        features['moi_principal_1'] = np.nan
        features['moi_principal_2'] = np.nan
        features['moi_principal_3'] = np.nan
    
    # ===== RADIAL FEATURES (2) =====
    if len(positions) >= 2:
        com = positions.mean(axis=0)
        distances = np.linalg.norm(positions - com, axis=1)
        features['rdf_mean'] = float(distances.mean())
        features['rdf_kurtosis'] = float(kurtosis(distances))
    else:
        features['rdf_mean'] = np.nan
        features['rdf_kurtosis'] = np.nan
    ...
    if len(positions) >= 2:
        ...
        features['entropy_spatial'] = float(entropy(H_norm))
    else:
        features['entropy_spatial'] = np.nan
    ...
    if len(positions) >= 10:
        try:
            ...
            features['ms_bandwidth'] = float(bandwidth)
        except:
            features['ms_bandwidth'] = np.nan
    else:
        features['ms_bandwidth'] = np.nan
...
def get_model_feature_names(model):
    """
    Detecta qué features usa el modelo.
    Compatible con sklearn y otros modelos.
    """
    # Intenta obtener nombres de features
    if hasattr(model, 'feature_names_in_'):
        return list(model.feature_names_in_)
    elif hasattr(model, 'n_features_in_'):
        # Si no tiene nombres, lo mínimo es que tenga número de features
        n_features = model.n_features_in_
        return None  # Retorna None para indicar que no sabemos los nombres
    else:
        return None

def extract_features_for_model(positions, model):
    """
    Extrae features de la forma que el modelo los necesita.
    """
    if len(positions) < 3:
        raise ValueError("No hay suficientes átomos")
    
    normalized_pos, box_size = normalize_positions(positions)
    
    # Calcular TODOS los features posibles
    all_features = calc_all_possible_features(normalized_pos, box_size)
    
    # Detectar qué features necesita el modelo
    model_features = get_model_feature_names(model)
    
    if model_features:
        # Si sabemos los nombres, seleccionar solo esos
        filtered_features = {k: all_features[k] for k in model_features if k in all_features}
        
        # Verificar que tenemos todos
        if len(filtered_features) < len(model_features):
            missing = set(model_features) - set(all_features.keys())
            st.warning(f"⚠️ Features faltantes en extracción: {missing}")
        
        return filtered_features
    else:
        # Si no sabemos nombres, retornar todos y dejar que sklearn maneje
        return all_features
...
def process_single_file(file_content: str, filename: str, model) -> Dict:
    ...
    try:
        ...
        features = extract_features_for_model(positions, model)
        
        # Crear DataFrame con los features en el orden correcto
        X_pred = pd.DataFrame([features])
        
        # Si el modelo tiene feature_names_in_, asegurar el orden
        if hasattr(model, 'feature_names_in_'):
...
with col2:
    ...
    st.markdown("<p style='text-align: center;'>Compatible con cualquier modelo</p>", 
                unsafe_allow_html=True)
...
VacancyPredict ML v2.1 | Compatible con cualquier modelo | Batch Processing
</div>
""", unsafe_allow_html=True)