...
class AlphaShapeSurfaceConstructor:
    ...
    def _build_mesh(self, delaunay, surface_facets, valid_tets):
        ...
        Returns vertices (positions from input atoms) and face indices.
        """
        if not surface_facets:
            return np.array([]), np.array([])
        
        # Surface vertices are those that appear in surface facets
        vertex_indices = sorted(set(np.array(surface_facets).flatten()))
        vertex_map = {old_idx: new_idx for new_idx, old_idx in enumerate(vertex_indices)}
        
        # Map vertices
        vertices = self.positions[vertex_indices]
    ...
    def get_surface_atoms(self, distance_cutoff=None):
        """
        Get atoms near the surface (within distance_cutoff from surface).
        If distance_cutoff is None, returns atoms marked as on surface.
        """
        ...
        # Compute distance of each atom to surface mesh
        distances = self._compute_particle_surface_distances()
        return np.where(distances <= distance_cutoff)[0]
    
    def _compute_particle_surface_distances(self):
        """
        Compute minimum distance from each input particle to surface mesh.
        """
        distances = np.zeros(len(self.positions))
        ...
        for i, pos in enumerate(self.positions):
            ...
            for face in self.surface_faces:
                ...
                dist = self._point_to_triangle_distance(pos, v0, v1, v2)
...
if __name__ == "__main__":
    ...
    # Example: Create synthetic FCC-like structure with a pore
    np.random.seed(42)
    positions = np.random.rand(50, 3) * 10
    
    # Run Alpha Shape construction
    ...
    print(f"\nResults:")
    print(f"Surface vertices: {len(constructor.surface_vertices)}")
    print(f"Surface faces: {len(constructor.surface_faces)}")
    print(f"Surface area: {constructor.surface_area:.4f}")
    print(f"Surface particles: {np.sum(constructor.surface_particle_selection)}")